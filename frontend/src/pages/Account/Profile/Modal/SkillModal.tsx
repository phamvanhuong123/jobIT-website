...
import { fetchUpdateCandidate } from "~/features/candidate.slice";
    ...
    type SkillModalProps = {
        visible: boolean;
        onClose: () => void;
        onSave?: (data: { hardSkills: { skill: string; experience: string }[]; softSkills: [] }) => void;
    ...
    const SkillModal: React.FC<SkillModalProps> = ({
        ...
        useEffect(() => {
            if (visible) {
                form.setFieldsValue({ hardSkills: skillsList, softSkills: softSkillsList });
            } else {
                form.resetFields();
        ...
        const handleOk = async () => {
            try {
                ...
                console.log(values)
                if (values.hardSkills !== 0){
                    values.skills = {
                        technicalSkill : values.hardSkills
                    }
                }
